...
# Sade mod: 46 özellik 456 satır için fazla. Bu liste kullanılınca
# sadece en güçlü sinyaller kalır. Kapatmak için SADE = False yap.
SADE = True
SADE_OZELLIKLER = [
    "begeni_momentum", "son6_egim", "pay_son_6s", "ivme",
    "oran_24_12", "oran_12_6", "oran_6_3",
    "log_kum_24s", "log_kum_1s",
    "begeni_oran", "yorum_oran", "log_yorum_24s",
    "log_sure", "kanala_gore_oran", "kanal_ustu_performans",
]
...
def egit(df: pd.DataFrame):
    ...
    if SADE:
        secili = [c for c in SADE_OZELLIKLER if c in X.columns]
        print(f"\nSade mod: {X.shape[1]} özellikten {len(secili)} tanesi kullanılıyor")
        X = X[secili]
    ...
    if "kanal_id" in df.columns and df["kanal_id"].notna().any():
        # Boş kanal_id'ler GroupKFold'u kırar; her birine kendi grubunu ver
        gruplar = df["kanal_id"].astype("object")
        bos = gruplar.isna()
        if bos.any():
            print(f"\n  {bos.sum()} satırın kanal_id'si boş — her biri ayrı "
                  f"grup sayıldı (join eşleşmemiş olabilir)")
            gruplar = gruplar.where(~bos, "bilinmeyen_" + df.index.astype(str))
        gruplar = gruplar.astype(str)

        kanal_sayisi = gruplar.nunique()
        katlar = list(GroupKFold(n_splits=min(KAT_SAYISI, kanal_sayisi))
                      .split(X, y, groups=gruplar))
        tekrar_sayisi = 1
        df = df.assign(kanal_id=gruplar)
    ...
    params = dict(
        ...
        lambda_l2=3.0,
        n_estimators=3000,
        importance_type="gain",    # kaç kez bölündü değil, ne kadar hata azalttı
        verbose=-1,
        random_state=TOHUM,
    )
    ...
    for tr, va in katlar:
        ...
        dur = [lgb.early_stopping(100, verbose=False)]
        try:
            # LightGBM 4.6+ imzası
            model.fit(X.iloc[tr], y[tr],
                      eval_X=X.iloc[va], eval_y=y[va], callbacks=dur)
        except TypeError:
            # eski sürümler
            model.fit(X.iloc[tr], y[tr],
                      eval_set=[(X.iloc[va], y[va])], callbacks=dur)
    ...
    # ---------- karışım: modeli tabana doğru çek ----------
    # Model zayıfsa tahmini medyan çarpana doğru büzmek hatayı düşürür.
    # a=1 saf model, a=0 saf taban.
    log_medyan = np.log(np.median(carpan))
    en_iyi = (1.0, ape_model.mean())
    print("\nKarışım denemesi (model ağırlığı → MAPE):")
    for a in (0.3, 0.5, 0.7, 0.85, 1.0):
        karisim = np.exp(a * oof + (1 - a) * log_medyan) * guvenli_v24
        m = (np.abs(karisim - y_ham) / np.maximum(y_ham, 1) * 100).mean()
        isabet = (np.abs(karisim - y_ham) / np.maximum(y_ham, 1) <= 0.20).mean() * 100
        print(f"  a={a:4.2f} → MAPE {m:5.2f}% | ±%20 içinde {isabet:5.1f}%")
        if m < en_iyi[1]:
            en_iyi = (a, m)

    alfa = en_iyi[0]
    if alfa < 1.0:
        print(f"\n  En iyi karışım a={alfa} → tahminler buna göre üretiliyor")
        tahmin = np.exp(alfa * oof + (1 - alfa) * log_medyan) * guvenli_v24
        ape_model = olc(y_ham, tahmin, "MODEL — karışım uygulanmış")
    ...
    if "kanal_id" in df.columns:
        kanal_ozet = (sonuc.assign(kanal_id=df["kanal_id"].values,
                                   mutlak_hata=sonuc["hata_yuzde"].abs())
                      .groupby("kanal_id")
                      .agg(video=("video_id", "size"),
                           ort_hata=("mutlak_hata", "mean"),
                           medyan_hata=("mutlak_hata", "median"),
                           yanlilik=("hata_yuzde", "mean"))
                      .round(1)
                      .sort_values("ort_hata", ascending=False))
        print(f"\nKanal bazında hata (en kötü 8 / {len(kanal_ozet)} kanal):")
        print(kanal_ozet.head(8).to_string())
        print(f"\nKanal ortalama hatalarının dağılımı: "
              f"en iyi {kanal_ozet['ort_hata'].min():.1f}% | "
              f"medyan {kanal_ozet['ort_hata'].median():.1f}% | "
              f"en kötü {kanal_ozet['ort_hata'].max():.1f}%")
        kanal_ozet.to_csv("kanal_bazinda_hata.csv")
...
def tahmin_et(df_yeni: pd.DataFrame, model_yolu: str = "model.pkl") -> np.ndarray:
    ...
    alfa = paket.get("alfa", 1.0)
    log_medyan = np.log(paket["medyan_carpan"])
    log_carpan = alfa * log_carpan + (1 - alfa) * log_medyan
